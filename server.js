...
function checkAvailableQualities(formats) {
  ...
  formats.forEach(f => {
    if (f.acodef && f.acodef !== 'none') q.audio = true;
    if (f.height) {
      if (f.height >= 4320) {
        q['8k'] = true;
...
function getFormatString(quality, merge) {
  ...
  const map = {
    ...
    'audio': 'bestaudio[ext=m4a]/bestaudio[acodef=aac]/bestaudio'
  };
...
app.post('/api/download-quality', (req, res) => {
  ...
  checkQualityAvailability(normalisedUrl, quality, merge, ytdlpPath)
    .then(({ isAvailable, availableQualities, formatString }) => {
      if (!isAvailable) {
        return res.status(400).json({ error: 'Requested quality not available for this video', availableQualities });
      }
